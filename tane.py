...
def compute_dependencies(level, listofcols):
    ...
    for x in level:
        for a in x:
            if a in dictCplus[x]:
                if eps > 0:
                    if validApproxfd(x.replace(a,''), a): # line 5
                        finallistofFDs.append([x.replace(a,''), a]) # line 6
                        dictCplus[x].remove(a)  # line 7

                        if validfd(x.replace(a,''), a): #! added this
                            listofcols=listofcolumns[:]
                            for j in x: # this loop computes R\X
                                if j in listofcols: listofcols.remove(j)
...
def validApproxfd(y,z): #! added this function
    global eps
    if y=='' or z=='': return False
    ey = computeE(y)
    eyz = computeE(y+z)
    if abs(ey-eyz) < eps: #! important
        return True
    else:
        return False
...
if __name__=="__main__":
    # modified dataset from class lectures
    data2D = pd.read_csv('D:\Western\CS9866\FD_CFD_extraction\people.csv')
    print(data2D.info())
